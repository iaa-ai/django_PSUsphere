...
class Command(BaseCommand):
    ...
    def create_organization(self, count):
        ...
        for i in range(count):
            words = [fake.word() for j in range(2)] # two words
            organization_name = ' '.join(words)
            Organization.objects.create(name=organization_name.title(),
    ...
    def create_students(self, count):
        fake = Faker('en_PH')
        for i in range(count):
            Student.objects.create(student_id=f"{fake.random_int(2020,2024)}-{fake.random_int(1,8)}-{fake.random_number(digits=4)}",
    ...
    def create_membership(self, count):
        fake = Faker()
        for i in range(count):
            OrgMember.objects.create(student=Student.objects.order_by('?').first(),
